...
public class HpbPropertyParser
{
    ...
    public List<HpbProperty> Parse(string contentArea)
    {
        try
        {
            ...
            if (tableNodes == null)
            {
                throw new InvalidOperationException(
                    "There was an error getting any 'table' nodes from the HTML list. The HTML structure has probably changed!");
            }
            ...
            foreach (var tableNode in tableNodes)
            {
                ...
                foreach (var trNode in trNodes)
                {
                    ...
                    var propertyTypeTrNode = propertyTypeTrNodes.SingleOrDefault(p =>
                        p.ChildNodes[1].InnerText ==
                        HpbHelpers.GetTextFromSearchBedrooms(_hpbScraperOptions.SearchBedrooms));
        ...
        {
            Log.Error(ex, $"The following content area could not be parsed: {contentArea}");
        }

        return new List<HpbProperty>();
    }
